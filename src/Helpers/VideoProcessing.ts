...
const createSegment = (
  ...
  return new Promise((resolve, reject) => {
    const command = ffmpeg(inputVideo)
      ...
      .format('mp4')
      .outputOptions([
        `-vf`, `drawtext=text='${textOnVideo ? `Episode ${episode} Part ${videoNumber}` : "1 million views"}':fontcolor=white:fontsize=80:box=1:boxcolor=black@0.5:boxborderw=20:x=(w-text_w)/2:y=50`,
        '-b:a', '64k',
    ]);
